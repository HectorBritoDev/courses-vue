...
class Language
{
    ...
    public function handle($request, Closure $next)
    {
        if (session('applocale')) {
            ...
            Carbon::setLocale(session('applocale')); //Da formato a las fechas segun el idioma
            App::setLocale(session('applocale')); //Da el idioma seleccionado
        } else {
            session()->put('applocale', config('app.fallback_locale'));
            setlocale(LC_TIME, 'es_ES.utf8');
            Carbon::setLocale(session('applocale'));
            App::setLocale(config('app.fallback_locate'));
