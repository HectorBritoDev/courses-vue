<template>
  <div>
    ...
    <v-server-table ref="table" :columns="columns" :url="url" :options="options">
      ...
      <div slot="status" slot-scope="props">{{ formattedStatus(props.row.status) }}</div>
  ...
  methods: {
    filterByStatus() {
      parseInt(this.status) !== 0
        ? Event.$emit("vue-tables.filter::status", this.status)
        : null;
    },
    formattedStatus(status) {
      const statuses = [null, "Publicado", "Pendiente", "Rechazado"];
